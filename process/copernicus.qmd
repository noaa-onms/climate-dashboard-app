...
title: "Copernicus Data Extraction"
subtitle: "metadata parameters: *`r params$yml`*"
---
...
# DEBUG
m_yml <- here("meta/copernicus_phy.yml")
params <- yaml::read_yaml(m_yml) 
params$proc_prod <- path_ext_remove(basename(m_yml))
proc <- strsplit(params$proc_prod, "_")[[1]][1]  # erddap or copernicus
prod <- strsplit(params$proc_prod, "_")[[1]][2]  # copernicus product

dir_out <- here(glue("data/{params$proc_prod}"))
dir_create(dir_out)
...
get_dataset_times <- function(dataset_description){
  dataset_version <- pluck(
    dataset_description, "products", "datasets", 1, "versions", 1, "label")
  services <- pluck(
