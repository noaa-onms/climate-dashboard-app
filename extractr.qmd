...
sanctuary_years |> 
  ...
  # future_pmap(
  pmap(
    \(nms, year){   #  nms = "CBNMS"; year = 2011   # DEBUG
...
# for (csv in list.files(here("data/noaa_sss/CBNMS"), pattern = "\\.csv$", full.names = T)) {
#   read_csv(csv) |> 
#     select(-any_of("nms")) |> 
#     write_csv(csv)
# }
...
## fix: NOAA_DHW -> noaa_sst

Whoah, they're slightly different

```{r}

r_crw <- rast(here("data/noaa_sst/CBNMS/1985.tif"))
r_dhw <- rast(here("data/NOAA_DHW/CBNMS/1985.tif"))

nlyr(r_crw)
plot(r_crw[[nlyr(r_crw)]])
plot(r_dhw[[nlyr(r_dhw)]])

names(r_crw)
names(r_dhw)

d_crw <- read_csv(here("data/noaa_sst/CBNMS/1985.csv"))
d_dhw <- read_csv(here("data/NOAA_DHW/CBNMS/1985.csv"))
